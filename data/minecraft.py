# 该脚本负责处理将数据中的x,y,z,yaw,pitch转换为c2w的相机外参

import numpy as np
import cv2

def minecraft_pose_to_Tc2w(x,y,z,yaw_deg,pitch_deg):
    ...
    C_w = np.array([x, y, z], dtype=np.float32)
...
def act_to_cameras(act):
    # 首先 x,y,z,yaw,pitch在act[:, 25-29]
    viewmats = []
    for act_frame in act:
        x = act_frame[27]
        y = act_frame[28]
        z = act_frame[29]
        yaw = act_frame[25]
        pitch = act_frame[26]
        viewmat = minecraft_pose_to_Tc2w(x,y,z,yaw,pitch)
        viewmats.append(viewmat)
    viewmats = np.stack(viewmats)
    return viewmats
    

def get_K(resizee_width, resized_height):
    ...
    sx = resizee_width / default_resolution[0]
    sy = resized_height / default_resolution[1]
...
ACTION_KEYS = [
    ...
    "cameraX", # 23
    "cameraY", # 24 
    "yaw", # 25 # 注意 action维度还是25， 后面五项只用来得到viewmats
    "pitch", # 26
    "xpos", # 27
    "ypos", # 28
    "zpos", # 29
...
def json_action_to_env_action(json_action):
    ...
    env_action["yaw"] = json_action["yaw"]
    env_action["pitch"] = json_action["pitch"]
    env_action["xpos"] = json_action["xpos"]
    env_action["ypos"] = json_action["ypos"]
    env_action["zpos"] = json_action["zpos"]
...
def read_obs_slice_decord(
    ...
    try:
        ...
        frames = video.get_batch(range(start, stop)).asnumpy()
